# Guide de déploiement de l'Edge Function verify-donation-qr

## Option 1: Déploiement via le Dashboard Supabase (Recommandé)

1. **Allez dans votre dashboard Supabase**
   - https://xgdinqpxjywlfhjylktu.supabase.co

2. **Naviguez vers Edge Functions**
   - Menu de gauche > Edge Functions

3. **Créez une nouvelle fonction**
   - Cliquez sur "New Edge Function"
   - Nom: `verify-donation-qr`
   - Remplacez le code par défaut par le contenu de:
     `supabase/functions/verify-donation-qr/index.ts`

4. **Copiez le code de l'Edge Function**
```typescript
...
6. **Vérifiez le déploiement**
   ...
   - URL: `https://xgdinqpxjywlfhjylktu.supabase.co/functions/v1/verify-donation-qr`

## Option 2: Déploiement via CLI (si vous voulez configurer la CLI)

1. Installez la CLI Supabase:
   ```bash
   # Via npm (non recommandé selon l'erreur)
   # npm install -g supabase
   
   # Via autres méthodes: https://github.com/supabase/cli#install-the-cli
   ```
...
3. Déployez:
   ```bash
   supabase functions deploy verify-donation-qr
   ```

## Après le déploiement

Une fois l'Edge Function déployée, vous pouvez tester le flux complet:

1. **Démarrez l'admin web**:
   ```bash
   cd admin_web
   npm run dev
   ```
...
3. **Testez**:
   - Connectez-vous à l'app mobile
   - Allez dans Profil > "Mon QR Code"
   - Connectez-vous à l'admin web (en tant qu'admin)
   - Allez dans `/admin/scan-qr`
   - Scannez le QR code affiché sur mobile
