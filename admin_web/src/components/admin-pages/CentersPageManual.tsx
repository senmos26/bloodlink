...
export default function CentersPageManual() {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-slate-500">Creez, modifiez et activez les centres BloodLink.</p>
      </div>

      <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
        Pour creer un responsable, ajoutez d&apos;abord l&apos;utilisateur dans Supabase Auth, puis dans Profils changez son role en center_admin. Ensuite, assignez-le ici au centre.
      </div>

      <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600">
        Une ville peut contenir plusieurs centres. Le responsable est lie a un centre precis, pas seulement a une ville.
      </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-semibold text-slate-900">Nouveau centre</h2>
            <p className="mt-1 text-sm text-slate-500">
              Creez un centre puis assignez un responsable existant avec le role center_admin.
            </p>
          </div>
          {editingCenterId ? (
            <button type="button" onClick={() => fillForm()} className="text-sm text-slate-500 underline">
              Annuler l&apos;edition
            </button>
          ) : null}
        </div>
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          ...
          <label className="text-sm text-slate-700">
            ...
            <select
              value={form.admin_id}
              onChange={(event) => setForm((current) => ({ ...current, admin_id: event.target.value }))}
              className="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-red-400"
            >
              <option value="">Aucun responsable</option>
              {admins.map((admin) => (
                <option key={admin.id} value={admin.id}>
                  {admin.phone ? `${admin.full_name} - ${admin.phone}` : admin.full_name}
                </option>
              ))}
            </select>
          </label>
              ...
              <div key={center.id} className="rounded-2xl border border-slate-200 px-5 py-5">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      Responsable: {center.admin_id ? adminMap.get(center.admin_id) ?? center.admin_id : "Aucun"}
                    </p>
                    <p className="mt-1 text-xs uppercase tracking-wide text-slate-400">Center ID: {center.id}</p>
                  </div>
                  <span
                    className={`rounded-full px-3 py-1 text-xs font-medium uppercase tracking-wide ${
                      center.is_active ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-600"
                    }`}
                  >
